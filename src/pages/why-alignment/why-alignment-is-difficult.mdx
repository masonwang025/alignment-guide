...
<Toggle title={<div>Anything it doesn’t value will be lost forever. How can we ensure that a powerful AI system doesn’t take actions that conflict with things humans care about?</div>}>
    ...
    More specifically, an agent that only cares about a subset of the variables in a system will be willing to trade off <span class="text-highlight-dark">arbitrarily large amounts</span> of any variables that *aren’t* part of its goal for arbitrarily tiny increases in any of the things that *are* in its goal.
...
<Toggle title={<div>But that scenario is unrealistic in many ways. One important way is that you wouldn’t be able to “turn it off.”</div>}>
